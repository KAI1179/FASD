...
class DatasetWrapper(Dataset):
    ...
    def __getitem__(self, i):
        train_or_test = self.base_dataset.root.split('/')[-1]
        img, target = self.base_dataset[self.indices[i]]
        if train_or_test == 'train':
            img_transformed = []
            transform_flip = self.base_dataset.transform.transforms[1]
            img_transformed.append(img)
            img_transformed.append(transform_flip(img.clone().detach()))
